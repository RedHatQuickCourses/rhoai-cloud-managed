...
[TIP]
This environment is created 3x16cpu workers nodes each with 64GB memory which allows for the deployment of medium sized containers with memory requirements of 32Gb or less. These provide excellent performance for most non-model training tasks. 
